...
# ----------------------------------------------------------------------
# Make this file work under both TF1 and TF2
# ----------------------------------------------------------------------
if hasattr(tf, "compat") and hasattr(tf.compat, "v1") and hasattr(
    tf.compat.v1, "disable_eager_execution"
):
    # On TF2: use the TF1-style graph API
    tf.compat.v1.disable_eager_execution()
    tf = tf.compat.v1
...
class GetMatrixForBPNet:
    ...
    def __init__(self, test_H, loc_nzero_row):
        ...
        self.loc_nzero_line = np.append(
            [np.mod(self.loc_nzero2, self.n)], [self.loc_nzero2 // self.n], axis=0
        )
    ...
    def get_Matrix_VC(self):
        ...
        H_sum_by_V_to_C = np.zeros(
            [self.num_all_edges, self.num_all_edges], dtype=np.float32
        )
        ...
        for i in range(0, self.num_all_edges):
            # np.where returns a tuple; take the first index explicitly
            idx = np.where(self.loc_nzero1 == self.loc_nzero2[i])[0]
            if idx.size == 0:
                raise ValueError(
                    "get_Matrix_VC: no match found for edge index %d" % i
                )
            Map_row_to_line[i] = idx[0]

        map_H_row_to_line = np.zeros(
            [self.num_all_edges, self.num_all_edges], dtype=np.float32
        )
        ...
        return (
            H_x_to_xe0,
            np.matmul(H_sum_by_V_to_C, map_H_row_to_line),
            np.matmul(H_xe_last_to_y, map_H_row_to_line),
        )

    ###################################################################################################
    def get_Matrix_CV(self):

        H_sum_by_C_to_V = np.zeros(
            [self.num_all_edges, self.num_all_edges], dtype=np.float32
        )
        ...
        for i in range(0, self.num_all_edges):
            # np.where returns a tuple; we only want the first matching index
            idx = np.where(self.loc_nzero4 == self.loc_nzero5[i])[0]
            if idx.size == 0:
                raise ValueError(
                    "get_Matrix_CV: no match found for edge index %d" % i
                )
            Map_line_to_row[i] = idx[0]

        map_H_line_to_row = np.zeros(
            [self.num_all_edges, self.num_all_edges], dtype=np.float32
        )
...
class BP_NetDecoder:
    def __init__(self, H, batch_size):
        ...
        self.llr_placeholder = tf.placeholder(
            tf.float32, [batch_size, self.v_node_num]
        )
        (
            self.llr_into_bp_net,
            self.xe_0,
            self.xe_v2c_pre_iter_assign,
            self.start_next_iteration,
            self.dec_out,
        ) = self.build_network()
        # transpose the llr matrix to adapt to the matrix operation in BP net decoder.
        self.llr_assign = self.llr_into_bp_net.assign(
            tf.transpose(self.llr_placeholder)
        )
    ...
    def __del__(self):
        # Only close the session if it was actually created
    ...
    def one_bp_iteration(self, xe_v2c_pre_iter, H_sumC_to_V, H_sumV_to_C, xe_0):
        ...
        xe_sum_log_img = tf.matmul(
            H_sumC_to_V, tf.multiply(tf.truediv((1 - xe_tanh_temp), [2.0]), [3.1415926])
        )
        xe_sum_log_real = tf.matmul(
            H_sumC_to_V, tf.log(1e-8 + tf.abs(xe_tanh))
        )
    ...
    def build_network(self):  # build the network for one BP iteration
        # BP initialization
        llr_into_bp_net = tf.Variable(
            np.ones([self.v_node_num, self.batch_size], dtype=np.float32)
        )
        xe_0 = tf.matmul(self.H_x_to_xe0, llr_into_bp_net)
        # the v->c messages of the previous iteration
        xe_v2c_pre_iter = tf.Variable(
            np.ones([self.num_all_edges, self.batch_size], dtype=np.float32)
        )
        ...
        xe_v_sumc, xe_c_sumv = self.one_bp_iteration(
            xe_v2c_pre_iter, H_sumC_to_V, H_sumV_to_C, xe_0
        )
        ...
        bp_out_llr = tf.add(
            llr_into_bp_net, tf.matmul(self.H_xe_v_sumc_to_y, xe_v_sumc)
        )
        dec_out = tf.transpose(
            tf.floordiv(1 - tf.to_int32(tf.sign(bp_out_llr)), 2)
        )
    ...
    def decode(self, llr_in, bp_iter_num):
        ...
        if real_batch_size != self.batch_size:  # padding zeros
            ...
            llr_in = np.append(
                llr_in,
                np.zeros(
                    [self.batch_size - real_batch_size, num_v_node],
                    dtype=np.float32,
                ),
                0,
            )
        ...
        for _ in range(0, bp_iter_num - 1):
